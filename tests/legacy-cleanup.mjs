...
const bannedEverywhere=['persistentFooter','waitingPromptText','finalMessage','computeSel','task-status--resumed','dataFollowup.reason','miningInspectionCount','factoryProductionComplete','migrateOldState','deriveCompatibilityState','copyCommonFlags','REWIND_FOR_PRE_V6','normalizeHeadingHierarchy','.flags.tookBreak','.flags.dcCoolingRestored'];
...
console.log(`Legacy cleanup guard passed: ${removedPaths.length} removed paths, ${bannedEverywhere.length} guarded tokens, ${actorIds.length} live supporting actors.`);