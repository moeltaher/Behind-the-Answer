...
const bannedEverywhere=['persistentFooter','waitingPromptText','finalMessage','computeSel','task-status--resumed','dataFollowup.reason'];
const obsoleteCurrentState=['.flags.tookBreak','.flags.dcCoolingRestored'];
...
for(const token of bannedEverywhere)for(const [path,source] of runtimeSources)if(source.includes(token))failures.push(`legacy runtime token ${token} found in ${path.slice(ROOT.length+1)}`);
// Old field names may appear only inside the compatibility reader in storage.js.
for(const token of obsoleteCurrentState)for(const [path,source] of runtimeSources){const relative=path.slice(ROOT.length+1);if(relative==='js/core/storage.js')continue;if(source.includes(token))failures.push(`obsolete current-state token ${token} found in ${relative}`);}
...
console.log(`Legacy cleanup guard passed: ${removedPaths.length} removed paths, ${bannedEverywhere.length+obsoleteCurrentState.length} guarded tokens, ${actorIds.length} live supporting actors.`);
