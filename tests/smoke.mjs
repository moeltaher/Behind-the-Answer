...
async function causalChecks(){
  ...
  await load(page,{scene:'mineTask',flags:{miningCount:6,miningMinutes:28,miningBUses:4,miningWarning:true,miningIncidentChoice:'continue',miningRiskLevel:2,miningForcedInspection:true}});await click(page,'#mineStop');let state=await saved(page);if(state.flags.miningInspectionMode!=='routine'||state.flags.miningInspectionCount!==0||!state.decisions.some(decision=>decision.id==='mine-forced-inspection-start'))throw new Error('Forced mining stop persisted an invalid or ambiguous pre-verification state.');await page.reload({waitUntil:'networkidle'});if(!await page.locator('#inspectMine').count())throw new Error('Forced mining inspection did not survive reload before inspection.');
