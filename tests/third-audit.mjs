...
console.log('THIRD_AUDIT:data-eligibility-v4');
...
if(!state.flags.dataTrainingApproved.includes(0)||state.flags.dataTrainingUsed.includes(0))throw new Error('v4 eligibility must record approval before historical use.');
...
const displayed=await page.locator('.full-evidence-details .decision-row').count();if(displayed!==decisions.length)throw new Error(`Full record lost decisions: ${displayed}/${decisions.length}.`);
await browser.close();console.log('Third-audit regression checks passed for schema v4.');
