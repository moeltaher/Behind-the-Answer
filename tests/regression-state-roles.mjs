...
function released(extra={}){return advanced({dataChecks:[{rights:'clear',privacy:'clear',fitness:'clear'},{rights:'clear',privacy:'clear',fitness:'clear'}],dataTrainingApproved:[],releaseGates:['regression','capacity','risk','rollback'],launchChoice:'ready',deployLoad:[45,30,25],deployFailoverChecks:[0,1,2],deployTabs:['network','compute','model'],deployRecovery:'rollback',deployRecoveryDisposition:'cleared',supportIndex:2,transferChoice:'build-use',...extra});}
function deploymentReady(loadValues=[45,30,25]){return released({deployLoad:loadValues,deployFailoverChecks:[],deployTabs:[],deployRecovery:null,deployRecoveryDisposition:null,supportIndex:0,transferChoice:null});}

if(STATE_SCHEMA_VERSION!==6)throw new Error(`Expected schema v6, got ${STATE_SCHEMA_VERSION}.`);
const browser=await chromium.launch();
const page=await browser.newPage({viewport:{width:1280,height:900}});

console.log('STATE_ROLES:retraining-revision-on-start');
await load(page,{scene:'governanceReview',flags:{...advanced({trainingCheckpoint:'recent',releaseGates:['regression','capacity','rollback']})}});
...
if(Object.hasOwn(state.flags,'trainingCompute')||state.flags.trainingCheckpoint!=='validated')throw new Error('Next revision retained obsolete compute state or failed to reset to the validated checkpoint.');
...
await load(page,{scene:'factoryOutcome',decisions:[{id:'factory-continue',label:'واصلت',effectText:'fixture'}],flags:{factoryChoice:'continue',factoryMaintenanceDebt:true,factoryRemediationStage:'none'}});
...
if(await page.locator('[data-task-panel][data-task-status="complete"]').count()!==1)throw new Error('Factory outcome did not become complete after diagnosis, repair and remeasurement.');
...
await load(page,{scene:'deployFailover',flags:{...deploymentReady([45,30,25])}});
...
await load(page,{scene:'deployFailover',flags:{...deploymentReady([60,5,35])}});
for(const index of [0,1,2])await click(page,`[data-failover-check="${index}"]`);
...
if(await supportDetails.getByText('مراجع لغة',{exact:true}).count())throw new Error('Conditional language reviewer appears despite the safety path not requiring it.');
...
await supportDetails.getByText(/تمثلها السلسلة/).waitFor({state:'visible'});
