...
if(htmlFiles.length!==1||relative(htmlFiles[0])!=='index.html')failures.push(`Runtime must have exactly one HTML entry page (index.html); found: ${htmlFiles.map(relative).join(', ')||'none'}.`);
const indexPath=join(ROOT,'index.html'),index=await readFile(indexPath,'utf8');
for(const match of index.matchAll(/(?:href|src)="(\.\/[^"?#]+)"/g)){const target=normalize(resolve(ROOT,match[1]));if(!fileSet.has(target))failures.push(`index.html references missing file ${match[1]}`);}
...
for(const field of ['annotationIndex','annotationCorrect','annotationAnswered','annotationCounts','trainingConfigured','trainingCompute'])if(Object.hasOwn(DEFAULT_STATE.flags,field))failures.push(`DEFAULT_STATE contains obsolete or derived flag: ${field}`);

const stateSourcePath=normalize(join(ROOT,'js/core/state.js'));
const runtimeOutsideState=[...runtimeText.entries()].filter(([file])=>file!==stateSourcePath).map(([,source])=>source).join('\n');
for(const field of Object.keys(DEFAULT_STATE.flags)){const dotRef=new RegExp(`\\.flags\\.${field}\\b`),bracketRef=new RegExp(`\\.flags\\[['\"]${field}['\"]\\]`);if(!dotRef.test(runtimeOutsideState)&&!bracketRef.test(runtimeOutsideState))failures.push(`Unused state flag: DEFAULT_STATE.flags.${field}`);}
...
const domainPath=normalize(join(ROOT,'js/domain/game-rules.js'));
for(const [file,source] of runtimeText.entries()){
  if(file===domainPath)continue;
  for(const signature of ['function hasUnresolved(','function unresolvedReadyIndices(','function exposedCurrentUnresolvedIndices(','function neededExtraChecks(','function confirmedAnnotations(','function survivableFailures(','const DEPLOY_CAPACITY_LIMITS =','const FAILOVER_INGRESS_LIMITS ='])if(source.includes(signature))failures.push(`Duplicated domain rule outside game-rules.js: ${relative(file)} contains ${signature.trim()}`);
}
const bannedRuntimePhrases=['post-training','workflow','restart أو rollback','(gold label)','history-each-time','interface-only'];
for(const [file,source] of runtimeText.entries())for(const phrase of bannedRuntimePhrases)if(source.includes(phrase))failures.push(`Obsolete or untranslated runtime phrase in ${relative(file)}: ${phrase}`);

const factories=[createIntroRoutes,createMiningRoutes,createFactoryRoutes,createDatacenterRoutes,createDataRoutes,createAnnotationRoutes,createTrainingRoutes,createEvaluationRoutes,createDeploymentRoutes,createEndingRoutes];
const registeredScenes=new Set(factories.flatMap(factory=>Object.keys(factory({}))));
const mappedScenes=Object.values(SCENES_BY_STAGE).flat(),mappedSet=new Set(mappedScenes);
if(mappedScenes.length!==mappedSet.size)failures.push('SCENES_BY_STAGE contains duplicate scene ids.');
for(const scene of mappedSet)if(!registeredScenes.has(scene))failures.push(`Mapped scene has no registered route: ${scene}`);
for(const scene of registeredScenes)if(!mappedSet.has(scene))failures.push(`Registered route has no stage mapping: ${scene}`);
const stageMapPath=normalize(join(ROOT,'js/data/stage-backgrounds.js'));
const transitionText=[...runtimeText.entries()].filter(([file])=>file!==stageMapPath).map(([,source])=>source).join('\n');
...
console.log(`Static integrity check passed across ${textFiles.length} text files and ${registeredScenes.size} routes, with one-page and legacy-rule guards.`);
