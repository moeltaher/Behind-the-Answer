...
const files=await walk(ROOT),fileSet=new Set(files.map(file=>normalize(file))),textFiles=files.filter(file=>TEXT_EXTENSIONS.has(extname(file))),sourceFiles=files.filter(file=>SOURCE_EXTENSIONS.has(extname(file))),runtimeSources=sourceFiles.filter(file=>relative(file).startsWith('js/')),failures=[],runtimeImports=new Map(),importedRuntimeNames=new Map(),referencedAssets=new Set(),runtimeText=new Map();
for(const file of sourceFiles){const source=await readFile(file,'utf8');if(relative(file).startsWith('js/'))runtimeText.set(normalize(file),source);for(const match of source.matchAll(/(?:from\s+|import\s*)['"]([^'"]+)['"]/g)){const target=localPath(file,match[1]);if(!target)continue;if(!fileSet.has(target)){failures.push(`${relative(file)} imports missing file ${match[1]}`);continue;}if(relative(file).startsWith('js/')&&relative(target).startsWith('js/')){const fromFile=normalize(file),targets=runtimeImports.get(fromFile)||new Set();targets.add(normalize(target));runtimeImports.set(fromFile,targets);}}for(const match of source.matchAll(/import\s*{([^}]+)}\s*from\s*['"]([^'"]+)['"]/gs)){const target=localPath(file,match[2]);if(!target||!relative(target).startsWith('js/'))continue;const names=importedRuntimeNames.get(target)||new Set();for(const name of importedNames(match[1]))names.add(name);importedRuntimeNames.set(target,names);}}
for(const file of files.filter(file=>file===join(ROOT,'index.html')||relative(file).startsWith('js/')||relative(file).startsWith('css/'))){const source=await readFile(file,'utf8');for(const match of source.matchAll(/assets\/images\/[^'"\s)]+/g))referencedAssets.add(normalize(resolve(ROOT,match[0])));}
...
for(const file of runtimeSources){if(!reachableRuntimeFiles.has(normalize(file)))failures.push(`Unused runtime JavaScript file unreachable from js/app.js: ${relative(file)}`);const source=runtimeText.get(normalize(file)),names=importedRuntimeNames.get(normalize(file))||new Set();for(const match of source.matchAll(/export\s+(?:const|let|var|function|class)\s+([A-Za-z_$][\w$]*)/g)){if(!names.has(match[1]))failures.push(`Unused runtime export: ${relative(file)}#${match[1]}`);}}
