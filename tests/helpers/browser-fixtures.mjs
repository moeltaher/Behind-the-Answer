...
export function stateWith(patch={}){
  ...
  if(atOrAfter(state.scene,'deployIncident')&&state.flags.deferredExtraChecks.length) {
    state.flags.deployMonitoringOpened=true;
    state.flags.monitoringChecksCompleted=[...state.flags.deferredExtraChecks];
  }
