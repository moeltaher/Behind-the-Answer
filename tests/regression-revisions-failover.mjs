...
await load(page,{scene:'transferChallenge',flags:{...baseAdvanced({releaseGates:['regression','capacity','risk','rollback'],launchChoice:'ready',deployLoad:[45,30,25],deployFailoverChecks:[0,1,2],deployTabs:['network','compute','model'],deployRecovery:'rollback',deployRecoveryDisposition:'cleared',supportIndex:2})}});
...
await load(page,{scene:'governanceReview',flags:{...baseAdvanced({dataChecks:[{rights:'unresolved',privacy:'clear',fitness:'clear'}],dataTrainingApproved:[0],releaseGates:[]})}});
await page.getByRole('heading',{name:'أغلق حواجز البيانات قبل مراجعة بوابات الإصدار.',exact:true}).waitFor({state:'visible'});
if(await page.locator('#toReleaseGates').count())throw new Error('Governance blocker allowed progression to release gates.');
if(!(await page.locator('[data-governance-remediate="0"]').count()))throw new Error('Governance blocker has no remediation action.');

console.log('REVISIONS_FAILOVER:retraining-waits-for-new-setup');
await load(page,{scene:'governanceReview',flags:{...baseAdvanced({dataIndex:2,dataStatuses:['ready','ready'],dataChecks:[{rights:'unresolved',privacy:'clear',fitness:'clear'},{rights:'clear',privacy:'clear',fitness:'clear'}],dataSort:{keep:2,remove:0,redact:0,review:0},dataTrainingApproved:[0],dataTrainingUsed:[0,1],dataCurrentTrainingUsed:[0,1],releaseGates:['regression','capacity','rollback']})}});
...
if(Object.hasOwn(retrained.flags,'trainingCompute')||retrained.flags.trainingCheckpoint!=='validated')throw new Error('Next revision retained obsolete compute state or failed to reset the checkpoint.');
...
for(const [values,expected] of [[[45,30,25],'1/3 حالات يمكن امتصاصها'],[[60,5,35],'0/3 حالات يمكن امتصاصها']]){
  await load(page,{scene:'deployFailover',flags:{...baseAdvanced({releaseGates:['regression','capacity','risk','rollback'],launchChoice:'ready',deployLoad:values})}});
