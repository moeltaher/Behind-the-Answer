...
function normalizeHeadingHierarchy(){for(const old of sceneEl.querySelectorAll('.response-card h4,.dual-view .view-panel h3')){const next=document.createElement('h2');for(const attribute of old.attributes)next.setAttribute(attribute.name,attribute.value);next.innerHTML=old.innerHTML;old.replaceWith(next);}}
function normalizeGuidanceCopy(markup){return markup.replaceAll('إطلاق مع مراقبة','قرار مع دين مراقبة').replaceAll('إطلاق بعد إغلاق الفحوص','قرار بعد إغلاق الفحوص').replaceAll('جاهز للإصدار','قرار الإصدار معتمد');}
...
function sceneHtml(content){const index=currentChapterIndex();renderJourneyProgress(ctx,index);promptBtn.hidden=index<0;updateBackdrop();const character=characterForScene(state.scene);const guidance=normalizeGuidanceCopy(sceneGuidance(state.scene,state));const notice=state.systemNotice;if(notice){state.systemNotice='';saveState();}const savedNotice=notice?`<div class="alert" role="status"><strong>تحديث الحفظ المحلي</strong><span>${h(notice)}</span></div>`:'';const warning=storageWarning();const failureNotice=warning?`<div class="alert dangerish" role="status"><strong>تنبيه الحفظ المحلي</strong><span>${h(warning)}</span></div>`:'';router.html(`${savedNotice}${failureNotice}${characterCard(character)}${guidance}${content}`);normalizeHeadingHierarchy();}
