...
const ACTORS = {
  mineOrientation:['supervisor'], mineTask:['supervisor','coworker'], mineInspection:['supervisor','maintenance'], transportMontage:['transportTeam'], abstract1:['transportTeam'],
  factoryOrientation:['maintenance','qualityInspector'], factoryMonitor:['qualityInspector'], factoryIncident:['maintenance','qualityInspector'], factoryOutcome:['qualityInspector'], hardwareMontage:['maintenance','qualityInspector'], abstract2:['maintenance','qualityInspector'],
  dcInstall:['electricalEngineer','networkOperator'], dcCooling:['coolingTech','electricalEngineer'], dcWorkers:['cleaner','electricalEngineer','securityWorker','coolingTech','cableTech','networkOperator'], abstract3:['coolingTech','electricalEngineer','networkOperator'],
  dataOrigins:['contentCreators'], dataClean:['contentCreators','dataReviewer'], dataCleanSummary:['contentCreators','dataReviewer'], abstract4:['contentCreators','dataReviewer'],
  annotationIntro:['dataReviewer'], annotationTask:['dataReviewer'], annotationReview:['dataReviewer'], annotationEnd:['dataReviewer'], abstract5:['dataReviewer'],
  trainingSetup:['infraTeam'], trainingRun:['infraTeam'], trainingEval:['infraTeam'],
  evalTask:['languageReviewer'], safetyTest:['safetyTester'], safetyOutcome:['safetyTester'], launchDecision:['releaseManager','safetyTester'], launchOutcome:['releaseManager'], abstract7:['safetyTester','languageReviewer'],
  deployLoad:['operationsTeam'], deployIncident:['operationsTeam'], onCall:['operationsTeam','affectedUser'], supportTask:['affectedUser'], deployEnd:['operationsTeam'], abstract8:['operationsTeam','affectedUser'],
  pipelineAssemble:['transportTeam','infraTeam','languageReviewer'], peopleReveal:['transportTeam','maintenance','contentCreators','safetyTester','operationsTeam']
};
...
function incidentGuidance(scene, state) {
  if (scene === 'mineTask' && state.flags.miningWarning) return actorMessage(supportingActor('coworker'),'لاحظت حركة غير معتادة في الجدار عند القطاع ب.','ينبّهك أثناء العمل') + eventPanel({title:'ظهر خطر في القطاع ب',trigger:'بعد استخراج 6 وحدات أشار زميل موسى إلى حركة غير معتادة في الجدار.',meaning:'لا يمكن مواصلة المهمة بصورة عادية قبل اتخاذ قرار بشأن الخطر.',actors:actorsFor(scene)}) + decisionPrompt('كيف ستتعامل مع خطر الجدار؟');
  ...
  if (scene === 'dcCooling') return actorMessage(supportingActor('coolingTech'),'وحدة التبريد رقم 3 لا تستجيب أثناء اختبار المجموعة.','يبلغ عن العطل') + eventPanel({title:'تعطلت وحدة تبريد أثناء اختبار المجموعة',trigger:'بعد تشغيل اختبار الخادم لم تعد وحدة التبريد رقم 3 تستجيب.',meaning:'العطل حدث أثناء الاختبار؛ يجب اختيار طريقة للتعامل معه قبل اعتبار المجموعة جاهزة.',actors:actorsFor(scene)}) + decisionPrompt('كيف ستتعامل مع عطل التبريد؟');
  if (scene === 'annotationReview' && state.flags.annotationCounts.rejected) return actorMessage(supportingActor('dataReviewer'),'رفضت مهمة واحدة وفق معيار المراجعة في المنصة.','يصدر قرار المراجعة') + eventPanel({title:'المراجع رفض إحدى المهام بعد انتهاء الوردية',trigger:'بعد إرسال التصنيفات راجع طرف آخر العمل ورفض مهمة واحدة.',meaning:'ظهر خيار الاعتراض لأن شخصًا آخر اتخذ قرارًا يؤثر في أجر أماني.',causal:'after',actors:actorsFor(scene)}) + decisionPrompt('هل ستعترض على قرار المراجع؟');
  ...
  if (scene === 'deployIncident') {
    const base = eventPanel({title:'ارتفعت نسبة الأخطاء أثناء اختبار الخدمة',trigger:'بعد توزيع الحمل وتشغيل الخدمة ظهرت أخطاء لدى مستخدمين، ويجب فحص الشبكة والخوادم وخدمة النموذج.',meaning:'افتح الأقسام الثلاثة أولًا. بعد معرفة المؤشرات ستظهر خيارات الاستعادة. لا تفترض اللعبة أن توزيعك للحمل هو سبب عطل الذاكرة.',actors:actorsFor(scene)});
...
function outcomeGuidance(scene, state) {
  ...
  if (scene === 'results') return taskOutcome({result:'ربطت واجهة الإجابة بالمراحل والأشخاص والقرارات التي سبقتها.',resume:'راجع الخاتمة أو دفتر السلسلة.',complete:true});
  return '';
}
...
export function sceneGuidance(scene, state) {
  ...
  return `${taskPanel(task,{status:statusFor(scene,state),progress:progressFor(stage,state),compact:false})}${actors.length ? actorStrip(actors) : ''}${contextualMessages(scene)}${incidentGuidance(scene,state)}${taskChoiceGuidance(scene)}${outcomeGuidance(scene,state)}`;
}
