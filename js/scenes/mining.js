...
export function createMiningRoutes(ctx) {
  ...
  function mineOrientation() {
    html(`<div><span class="eyebrow">موقع استخراج افتراضي</span><h1 class="scene-title">أنت الآن موسى، عامل استخراج وفرز.</h1><div class="reality-note"><strong>لماذا أنت هنا؟</strong> الأجهزة قد تعتمد على سلاسل تبدأ بخامات النحاس، والبوكسيت الذي يُنتج منه الألومنيوم، والسيليكا أو الكوارتز التي تدخل لاحقًا في إنتاج السيليكون، إضافة إلى مواد أخرى تختلف باختلاف الجهاز والمكونات.</div><p class="scene-subtitle">المطلوب تسليم 12 وحدة قبل مغادرة الشاحنة بعد ${SHIFT_LIMIT} دقيقة افتراضية. القطاع ب ينتج وحدتين في الوقت نفسه الذي تنتج فيه القطاعات الأخرى وحدة واحدة، لكنه يدخل حالة تحذير بعد تكرار استخدامه.</p><div class="hud-grid"><div class="hud-item"><span>الحصة</span><strong>12 وحدة</strong></div><div class="hud-item"><span>نافذة التسليم</span><strong>${SHIFT_LIMIT} دقيقة</strong></div><div class="hud-item"><span>الدخل الكامل</span><strong>100 وحدة لعب</strong></div></div><div class="reality-note"><strong>قواعد الأجر في السيناريو</strong> تجاوز نافذة التسليم يخفض الجزء المتغير 10 وحدات. إغلاق القطاع للفحص يخفضه 8 وحدات إضافية بسبب وقت التوقف. لذلك تخلق قواعد الأجر هنا حافزًا ماليًا لمواصلة العمل حتى عندما يظهر تحذير؛ هذه مفاضلة مقصودة في السيناريو وليست «إجابة صحيحة» مخفية.</div><div class="action-row"><button id="startMine" class="primary-btn">ابدأ العمل</button></div></div>`);
  ...
  function mineTask() {
    ...
    $('#mineContinue').addEventListener('click',()=>{
      ...
      state.flags.miningWarning=false;
      addDecision('mine-continue','أبقيت القطاع ب مفتوحًا بعد التحذير','حافظت على وقت الوردية والدخل، لكن الخطر بقي غير محسوم حتى نهايتها.');
      saveState();
      if(state.flags.miningCount>=12) go('mineEnd'); else mineTask();
    });
  }
  ...
  function mineEnd() {
    ...
    const incentiveNote=continued?'<div class="alert dangerish"><strong>ما الذي صنعه نظام الأجر؟</strong><span>الإبقاء على القطاع مفتوحًا حافظ على الجزء المتغير من الدخل في هذا المسار. اللعبة تسجل ذلك كحافز اقتصادي يدفع العامل نحو قبول مخاطرة أكبر، لا كخيار «أفضل» أخلاقيًا.</span></div>':'';
