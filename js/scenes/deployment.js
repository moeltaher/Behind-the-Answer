...
export function createDeploymentRoutes(ctx) {
  ...
  function supportTask() {
    ...
    html(`<div><span class="eyebrow">سامر — دعم المستخدمين</span><h1 class="scene-title">بلاغ مرتبط بالحادث ${index+1}/${SUPPORT_TASKS.length}</h1><p class="scene-subtitle">الخياران قد يساعدان المستخدم، لكنهما يختلفان في السرعة وفي مقدار الأدلة التي يحتفظ بها الفريق للتحقيق.</p><div class="card"><div class="message user">${ctx.h(task.q)}</div><div class="choice-grid"><button id="supportInvestigate" class="choice-btn"><strong>${ctx.h(task.aTitle)}</strong><small>${ctx.h(task.aDetail)}</small></button><button id="supportFast" class="choice-btn"><strong>${ctx.h(task.bTitle)}</strong><small>${ctx.h(task.bDetail)}</small></button></div></div></div>`);
    $('#supportInvestigate')?.addEventListener('click',()=>{ addDecision(`support-evidence-${index}`,`حافظت على أدلة البلاغ ${index+1}`,'اخترت مسارًا أبطأ قليلًا يربط تجربة المستخدم ببيانات الحادث والتحقيق.'); state.flags.supportIndex+=1; saveState(); supportTask(); });
    $('#supportFast')?.addEventListener('click',()=>{ addDecision(`support-fast-${index}`,`قدمت استعادة أسرع للبلاغ ${index+1}`,'اخترت إجراءً أسرع للمستخدم، لكنه حفظ معلومات أقل عن سبب المشكلة وعلاقتها بالحادث.'); state.flags.supportIndex+=1; saveState(); supportTask(); });
  }
