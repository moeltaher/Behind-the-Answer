...
export function createDeploymentRoutes(ctx) {
  ...
  function deployLoadAfterMonitoring() {
    if(Array.isArray(state.flags.deployLoad)) {
      ...
      $('#acceptFailoverRisk')?.addEventListener('click',()=>{const resilient=survivableFailures(values);addDecision(riskDecisionId(values),'قبلت فجوة المرونة صراحة قبل التشغيل',`التوزيع ${values.join(' / ')} يتحمل ${resilient}/3 حالات N‑1 فقط. القيد البنيوي مسجل ولم يُعرض كنجاح كامل.`);saveState();go('deployIncident');});
      $('#finishFailover')?.addEventListener('click',()=>{const resilient=survivableFailures(values);addDecision(`deploy-failover-review-${values.join('-')}`,'اختبرت التوزيع ضد خروج كل مركز مع حدود نقل الطوارئ',`تحمل التوزيع ${resilient} من 3 حالات كاملة؛ النتيجة تصف المرونة ولا تفسر سبب الحادث اللاحق.`);saveState();go('deployIncident');});
      return;
    }
