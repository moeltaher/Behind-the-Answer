...
export function createDeploymentRoutes(ctx) {
  ...
  function deployLoad() {
    ...
    $('#testLoad')?.addEventListener('click', () => {
      ...
      if (max > 60) {
        if (feedback) feedback.innerHTML = '<strong>المجموع صحيح، لكن الحمل ما زال مركزًا أكثر من اللازم.</strong><span>اخفض أعلى حصة إلى 60% أو أقل قبل الاختبار.</span>';
        tone(170, .08, 'square');
        return;
      }
  ...
  function deployIncident() {
    ...
    $('#restartInst')?.addEventListener('click', () => {
      ...
      addDecision('deploy-restart', 'أعدت تشغيل الوحدات المتأثرة', 'عادت الخدمة أسرع مع احتمال بقاء سبب العطل.', { pressure: 5, cost: -3, burden: 5, reliability: -3 });
      saveState();
      go('onCall');
    });
    $('#rollback')?.addEventListener('click', () => {
      state.flags.deployRecovery = 'rollback';
      addDecision('deploy-rollback', 'عدت إلى الإصدار السابق', 'تحملت وقت استعادة أطول واستهدفت الإصدار المشتبه به.', { pressure: -3, cost: 4, burden: 1, reliability: 5 });
      saveState();
      go('onCall');
    });
  }
  ...
  function supportTask() {
    ...
    $('#supportGood')?.addEventListener('click', () => {
      mutateMetrics({ serviceQuality: 3 });
      state.flags.supportIndex += 1;
      saveState();
      supportTask();
    });
    $('#supportAlt')?.addEventListener('click', () => {
      mutateMetrics({ pressure: 1, serviceQuality: -3 });
      state.flags.supportIndex += 1;
      saveState();
      supportTask();
    });
  ...
  function abstract8() {
    abstraction([['هانا', 'التشغيل', '◉'], ['سامر', 'الدعم', '☏'], ['فرق الشبكات', '', '≋']], 'الخدمة متاحة', 'المناوبات وإصلاح الأعطال ودعم المستخدمين أصبحت للمستخدم حالة واحدة: الخدمة تعمل.', 'ch9Intro');
  }
