...
export function createDeploymentRoutes(ctx) {
  ...
  function flexibleSites(values=savedLoad()) { return loadMargins(values).filter(value=>value>0).length; }
  ...
  function deployLoad() {
    const initial=savedLoad();
    html(`<div><span class="eyebrow">هانا — مهندسة تشغيل الخدمة</span><h1 class="scene-title">أصبحت الخدمة متاحة للمستخدمين.</h1><div class="reality-note"><strong>المهمة</strong> وزّع 100% من الحمل من دون تجاوز السعة التشغيلية المتاحة: أ 60%، ب 45%، ج 35%. توجد حلول صحيحة متعددة. اللعبة ستحفظ التوزيع نفسه لأن أثره المهم ليس «هل سبب العطل»، بل مقدار المرونة المتاحة أثناء الاستعادة.</div><div class="load-grid">${['أ','ب','ج'].map((name,index)=>`<div class="load-card"><span>مركز البيانات ${name}</span><strong id="load${index}">${initial[index]}%</strong><small>السعة المتاحة: ${CAPACITY_LIMITS[index]}%</small><input id="range${index}" type="range" min="0" max="100" step="1" value="${initial[index]}" aria-label="حصة مركز البيانات ${name}" /></div>`).join('')}</div><div id="loadFeedback" class="alert"><strong>المجموع: <span id="loadTotal">${initial.reduce((sum,value)=>sum+value,0)}</span>%</strong><span>لا يوجد توزيع وحيد صحيح. المطلوب احترام السعات وترك هامش في أكثر من موقع إن أمكن.</span></div><div class="action-row"><button id="testLoad" class="primary-btn">اختبر التوزيع</button></div></div>`);
    ...
    $('#testLoad')?.addEventListener('click',()=>{
      ...
      addDecision('deploy-capacity-load','وزعت الحمل ضمن سعات المراكز',zeroMargins?`نجح التوزيع، لكن ${zeroMargins} من المراكز يعمل عند حد السعة تمامًا؛ سيبقى ذلك ظاهرًا أثناء الاستعادة باعتباره مرونة أقل، من دون افتراض أنه سبب العطل.`:'نجح التوزيع وترك هامشًا في المراكز الثلاثة، ما يمنح مرونة أكبر إذا احتاج التشغيل إلى إعادة توزيع أثناء الاستعادة.');
  ...
  function deployIncident() {
    ...
    const verificationNote=state.flags.launchChoice==='fast'?'الإطلاق السابق ترك تحققًا إضافيًا للمراقبة بعد الإطلاق؛ هذا لا يثبت أنه سبب الحادث، لكنه يزيد أهمية جمع الأدلة الآن.':'حتى بعد إكمال التحقق قبل الإطلاق يمكن أن تظهر أعطال تشغيلية جديدة؛ الاختبارات تقلل عدم اليقين ولا تلغيه.';
    const values=savedLoad();
    const margins=loadMargins(values);
    const flex=flexibleSites(values);
    const capacityNote=`توزيعك السابق ${values.join(' / ')}% يترك هامشًا موجبًا في ${flex} من 3 مراكز (${margins.join(' / ')} نقاط مئوية). هذا لا يفسر سبب الحادث، لكنه يحدد مساحة المناورة إذا احتجت إلى نقل حمل أثناء الاستعادة.`;
    ...
    $('#restartInst')?.addEventListener('click',()=>{ const flex=flexibleSites(); state.flags.deployRecovery='restart'; addDecision('deploy-restart','أعدت تشغيل الوحدات المتأثرة',`عادت الخدمة أسرع، لكن السبب المشتبه به ظل في الإصدار نفسه. كان توزيعك يترك هامشًا في ${flex} مراكز، وهي معلومة تخص مرونة الاستعادة لا سبب العطل.`); saveState(); go('onCall'); });
    $('#rollback')?.addEventListener('click',()=>{ const flex=flexibleSites(); state.flags.deployRecovery='rollback'; addDecision('deploy-rollback','عدت إلى الإصدار السابق',`تحملت وقت استعادة أطول واستهدفت الإصدار المشتبه به. كان توزيعك يترك هامشًا في ${flex} مراكز للمناورة أثناء الاستعادة، من دون نسب السبب إلى التوزيع.`); saveState(); go('onCall'); });
  }

  function onCall() {
    const restarted=state.flags.deployRecovery==='restart';
    const values=savedLoad();
    html(`<div><span class="eyebrow">بعد استعادة الخدمة</span><h1 class="scene-title">الخدمة عادت، لكن أثر الحادث ما زال يصل إلى المستخدمين.</h1>${supportingRoleStrip(['affectedUser'],'من يظهر في الواجهة الآن؟')}<div class="card"><p>تعاملت هانا مع العطل ${restarted?'بإعادة تشغيل الوحدات':'بالعودة إلى الإصدار السابق'}. بقي توزيع الحمل الذي اخترته (${values.join(' / ')}%) جزءًا من حالة التشغيل، لا مجرد لغز انتهى. الآن تصل بلاغات مرتبطة بفترة التعطل، وبعض خيارات الدعم أسرع لكنها تحفظ أدلة أقل للتحقيق.</p></div><div class="action-row"><button id="toSupport" class="primary-btn">انتقل إلى بلاغات الحادث</button></div></div>`);
  ...
  function supportTask() {
    ...
    const values=savedLoad();
    html(`<div><span class="eyebrow">سامر — دعم المستخدمين</span><h1 class="scene-title">بلاغ مرتبط بالحادث ${index+1}/${SUPPORT_TASKS.length}</h1>${supportingRoleStrip(['affectedUser'],'صاحب البلاغ')} ${supportFeedbackMarkup()}<div class="reality-note"><strong>سياق الحادث المحفوظ</strong>التوزيع التشغيلي أثناء الحادث: ${values.join(' / ')}%. يبقى ظاهرًا هنا لأنه جزء من حالة التشغيل التي يحقق فيها الفريق، من دون اعتباره سببًا تلقائيًا للعطل.</div><p class="scene-subtitle">الخياران قد يساعدان المستخدم، لكنهما يختلفان في السرعة وفي مقدار الأدلة التي يحتفظ بها الفريق للتحقيق.</p><div class="card"><div class="message user">${ctx.h(task.q)}</div><div class="choice-grid"><button id="supportInvestigate" class="choice-btn"><strong>${ctx.h(task.aTitle)}</strong><small>${ctx.h(task.aDetail)}</small></button><button id="supportFast" class="choice-btn"><strong>${ctx.h(task.bTitle)}</strong><small>${ctx.h(task.bDetail)}</small></button></div></div></div>`);
  ...
  function deployEnd() {
    const values=savedLoad();
    addLedger(7,'هانا وسامر وفرق العمليات','توزيع حمل بحسب السعة، مراقبة، تشخيص، استعادة ودعم','الخدمة متاحة للمستخدمين',`حُفظ توزيع الحمل (${values.join(' / ')}%) واستخدم لتفسير مرونة الاستعادة من دون نسب سبب العطل إليه.`);
    html(`<div><span class="eyebrow">نهاية مرحلة التشغيل</span><h1 class="scene-title">اكتملت مراحل اللعب الثماني.</h1>${supportFeedbackMarkup()}<div class="stage-output"><strong>الخدمة عادت إلى حالة متاحة بعد عطل</strong>خلف كلمة «متاحة» توجد سعات مختلفة ومراقبة وتشخيص واستعادة ودعم وصيانة.</div><div class="hud-grid"><div class="hud-item"><span>توزيع الحمل المحفوظ</span><strong>${values.join(' / ')}%</strong></div><div class="hud-item"><span>طريقة الاستعادة</span><strong>${state.flags.deployRecovery==='rollback'?'العودة لإصدار سابق':'إعادة تشغيل الوحدات'}</strong></div><div class="hud-item"><span>بلاغات الحادث</span><strong>${state.flags.supportIndex} عولجت</strong></div><div class="hud-item"><span>العمل البشري في الواجهة</span><strong>غير ظاهر</strong></div></div><div class="action-row"><button id="uptimeAbstract" class="primary-btn">أغلق مرحلة التشغيل وارجع إلى إجابتك</button></div></div>`);
    $('#uptimeAbstract')?.addEventListener('click',()=>go('abstract8'));
  }

  function abstract8(){ abstraction([['هانا','مهندسة تشغيل','◉'],['سامر','دعم المستخدمين','◌'],['فرق المناوبة والصيانة','','◇']],'الخدمة متاحة','التوزيع والمراقبة والتشخيص والاستعادة والدعم أصبحت للمستخدم حالة بسيطة: «الخدمة متاحة»، مع بقاء قرارات السعة جزءًا من حالة التشغيل الفعلية.','pipelineAssemble'); }
