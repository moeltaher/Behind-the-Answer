...
export function createAnnotationRoutes(ctx) {
  ...
  function annotationSummary() {
    ...
    return { answered:results.length,accepted,pending,rejected,reasonable,agreement:results.length?Math.round((reasonable/results.length)*100):100 };
  }
  ...
  function annotationTask() {
    ...
    html(`<div class="annotation-shell"><div class="annotation-head"><strong>مشروع تصنيف البيانات</strong><span>المهمة ${index+1} من ${ANNOTATION_TASKS.length}</span></div><div class="annotation-body">${task.sensitive?'<div class="alert annotation-sensitive"><strong>تنبيه محتوى</strong><span>وصف مقتضب لمادة حساسة من دون تفاصيل صادمة.</span></div>':''}<div class="annotation-stats"><span>المكتملة: ${summary.answered}</span><span>التوافق المؤقت: ${summary.agreement}%</span><span>المبلغ المبدئي: ${provisionalEarnings()}</span><span>غير محسوم بعد المراجعة: ${summary.answered}</span></div><div class="annotation-copy">${ctx.h(task.text)}</div><div class="annotation-tags">${ANNOTATION_LABELS.map(label=>`<button data-tag="${ctx.h(label)}">${ctx.h(label)}</button>`).join('')}</div>${index===3&&!state.flags.tookBreak?'<div class="action-row"><button id="takeBreak" class="choice-btn break-btn"><strong>خذ استراحة</strong><small>تتوقف المهمة مؤقتًا ولا تضيف مهمة أو مبلغًا إلى الحساب المبدئي.</small></button></div>':''}</div></div>`);
