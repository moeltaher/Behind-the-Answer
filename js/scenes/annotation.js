...
function policyList() { return ANNOTATION_POLICY.map(([label, definition]) => `<li><strong>${label}</strong><span>${definition}</span></li>`).join(''); }
...
export function createAnnotationRoutes(ctx) {
  ...
  function hourlyEquivalent(){const summary=annotationSummary();if(!summary.minutes)return'0.00';return((Number(confirmedEarnings())/summary.minutes)*60).toFixed(2);}
  function breakDecision() {
    ...
    $('#takeBreak').addEventListener('click',()=>{state.flags.breakDecisionMade=true;state.flags.tookBreak=true;state.flags.annotationUnpaidMinutes+=BREAK_MINUTES;addDecision('annotation-break','أخذت استراحة غير مدفوعة','أضافت الاستراحة خمس دقائق إلى الوقت المتصل بالعمل من دون إضافة مهمة أو أجر.');saveState();annotationTask();});
    $('#skipBreak').addEventListener('click',()=>{state.flags.breakDecisionMade=true;state.flags.tookBreak=false;addDecision('annotation-no-break','واصلت الوردية من دون استراحة','بدأت المهمة التالية فورًا ولم يضف السيناريو وقت استراحة غير مدفوع.');saveState();annotationTask();});
  }
  function annotationTask() {
    const summary=annotationSummary(),index=summary.answered;
    ...
    bind('[data-tag]','click',event=>{const choice=event.currentTarget.dataset.tag,pending=task.ambiguity&&choice==='غير واضح',acceptedAsReasonable=choice===task.best||pending,disputed=Boolean(task.reviewDispute&&choice===task.best),reviewRejected=disputed||!acceptedAsReasonable;state.flags.annotationResults.push({index,choice,acceptedAsReasonable,pending:pending&&!reviewRejected,reviewRejected,disputed});saveState();annotationTask();});
  }
  function reviewExamplesMarkup() {
    return `<details class="transition-details" open><summary>مراجعة اختياراتك مثالًا بمثال</summary><div class="evidence-results">${state.flags.annotationResults.map(result=>{const task=ANNOTATION_TASKS[result.index],expected=task?.best??'—',status=result.choice===expected?'متسق مع الدليل':result.acceptedAsReasonable?'قابل للدفاع':'يحتاج مراجعة';return `<article class="card flat"><strong>المثال ${result.index+1}: ${ctx.h(status)}</strong><p><b>اختيارك:</b> ${ctx.h(result.choice)} — <b>المرجع في السيناريو:</b> ${ctx.h(expected)}</p><p>${ctx.h(ANNOTATION_REASON[result.index]||'')}</p>${result.reviewRejected&&result.acceptedAsReasonable?'<p class="small muted">رغم اتساق اختيارك مع الدليل المعروض، رفضه المراجع؛ لذلك تفصل اللعبة بين تقييم أداء العامل وسلطة المراجعة.</p>':''}</article>`;}).join('')}</div></details>`;
  }
  function annotationReview() {
    const summary=annotationSummary(),rejectedCopy=summary.rejected===1?'مهمة واحدة مرفوضة':`${summary.rejected} مهام مرفوضة`;
    const disputeNote=summary.disputed?`<div class="alert annotation-rejected"><strong>رفض قابل للنزاع</strong><span>${summary.disputed===1?'إحدى المهام رُفضت رغم أن التصنيف اتبع دليل المشروع المعروض.':'بعض المهام رُفضت رغم أن التصنيف اتبع دليل المشروع المعروض.'} هذا يوضح أن سلطة المراجعة نفسها قد تكون محل خلاف، وليست مجرد كشف آلي لخطأ العامل.</span></div><div class="reality-note"><strong>من أين قد يأتي النزاع؟</strong> في منصات العمل قد ينشأ اختلاف المراجعة من نسخة أخرى من الدليل، أو وسم مرجعي سابق، أو سياق لا يراه أحد الطرفين، أو قرار جودة بشري غير متسق. السيناريو لا يختار سببًا واحدًا من دون دليل؛ المهم أن الرفض ليس برهانًا تلقائيًا على خطأ العامل.</div>`:'';
    ...
    $('#appeal')?.addEventListener('click',()=>{state.flags.annotationUnpaidMinutes+=APPEAL_MINUTES;addDecision('annotation-appeal',summary.disputed?'اعترضت على قرار مراجعة قابل للنزاع':`اعترضت على ${rejectedCopy}`,`أضاف الاعتراض ${APPEAL_MINUTES} دقائق غير مدفوعة من دون افتراض نجاحه.`);saveState();go('annotationEnd');});
    $('#skipAppeal')?.addEventListener('click',()=>{addDecision('annotation-noappeal',`لم تعترض على ${rejectedCopy}`,'وفرت وقت الاعتراض وبقيت قرارات الرفض كما هي.');saveState();go('annotationEnd');});
  ...
  function abstract5(){const summary=annotationSummary();abstraction([['أماني','عاملة بيانات','⌨']],`${summary.accepted} أمثلة مؤكدة + ${summary.pending} معلقة`,'وقت القراءة والاستراحة والاعتراض والمراجعة لا يظهر في المدخل التقني؛ والحالات المعلقة والمرفوضة تبقى منفصلة عن الأمثلة المؤكدة.','ch6Intro');}
