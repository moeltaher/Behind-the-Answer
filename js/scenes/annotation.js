...
export function createAnnotationRoutes(ctx) {
  ...
  function breakDecision() {
    html(`<div class="annotation-shell"><div class="annotation-head"><strong>مشروع تصنيف البيانات</strong><span>قبل المهمة 4 من ${ANNOTATION_TASKS.length}</span></div><div class="annotation-body"><span class="eyebrow annotation-eyebrow">قرار وقت العمل</span><h1 class="annotation-review-title">وصلت إلى نقطة الاستراحة.</h1><p>في هذا السيناريو يمكنك أخذ خمس دقائق الآن، لكنها لا تُدفع. إذا واصلت، تنتقل مباشرة إلى المهمة التالية.</p><div class="choice-grid"><button id="takeBreak" class="choice-btn break-btn"><strong>خذ استراحة 5 دقائق</strong><small>تضاف إلى الوقت المتصل بالعمل من دون أجر.</small></button><button id="skipBreak" class="choice-btn annotation-light-choice"><strong>واصل من دون استراحة</strong><small>لا يضاف وقت الآن، وتبدأ المهمة الرابعة مباشرة.</small></button></div></div></div>`);
  ...
  function annotationTask() {
    ...
    bind('[data-tag]','click',event=>{
      ...
      state.flags.annotationResults.push({index,choice,acceptedAsReasonable,pending,reviewRejected,disputed});
      saveState();
      annotationTask();
    });
  }
  ...
  function abstract5(){
    const summary=annotationSummary();
    abstraction([['أماني','عاملة بيانات','⌨']],`${summary.accepted} مؤكدة + ${summary.pending} معلقة + ${summary.rejected} مرفوضة`,'وقت القراءة والاستراحة والاعتراض والمراجعة لا يظهر في المدخل التقني؛ ولا تنتقل إلى التطوير إلا الحالات المؤكدة.','ch6Intro');
  }
