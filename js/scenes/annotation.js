...
export function createAnnotationRoutes(ctx) {
  ...
  function annotationTask() {
    ...
    bind('[data-tag]', 'click', event => {
      ...
      const acceptedAsReasonable =
        choice === task.best || (task.ambiguity && choice === 'غير واضح');
  ...
  function annotationReview() {
    state.flags.annotationCounts.rejected =
      state.flags.annotationCounts.accepted > 0 ? 1 : 0;
    saveState();
    ...
    $('#skipAppeal')?.addEventListener('click', () => {
      addDecision(
        'annotation-noappeal',
        'لم تعترض على المهمة المرفوضة',
        'وفرت الوقت لكن قرار المنصة ظل قائمًا بلا مراجعة.',
