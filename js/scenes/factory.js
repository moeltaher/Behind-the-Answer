export function createFactoryRoutes(ctx) {
  ...
  function maintenanceResolved(){ return state.flags.factoryChoice==='stop' || state.decisions.some(decision=>decision.id==='factory-debt-closed'||decision.id==='factory-debt-carried'); }

  function factoryOutcome() {
    const stopped = state.flags.factoryChoice === 'stop';
    const carried = state.decisions.some(decision=>decision.id==='factory-debt-carried');
    ...
    const investigation = stopped
      ...
      : closedAfter
        ? '<div class="alert goodish"><strong>أُغلق دين الصيانة بعد انتهاء الدفعة</strong><span>بعد الحفاظ على موعد الشحن، أوقفت الفرق المعدة في نافذة صيانة منفصلة، حددت مصدر التسرب وأصلحته ثم تحققت من عودة المؤشر إلى النطاق. نجاح الفحص النهائي وحده لم يكن كافيًا؛ الإغلاق احتاج عمل صيانة مستقلًا.</span></div>'
    ...
    $('#carryMaintenance')?.addEventListener('click',()=>{state.flags.factoryMaintenanceDebt=true;addDecision('factory-debt-carried','نقلت دين الصيانة إلى الوردية التالية','بقي سبب ارتفاع الجسيمات غير مغلق رغم اجتياز المكونات للفحص النهائي.');saveState();factoryOutcome();});
