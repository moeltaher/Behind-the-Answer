...
export function createDataRoutes(ctx) {
  ...
  function dataClean() {
    ...
    bind('[data-sort]', 'click', event => {
      ...
      if (choice === item.recommended) mutateMetrics({ quality: 2 });
      ...
      if (item.recommended === 'review') {
        if (choice === 'review') {
          addDecision(`data-review-${item.type}`, 'أوقفت مادة حتى مراجعة مصدرها أو حقوق استخدامها', 'تحملت خطوة أبطأ بدل اعتبار الإتاحة التقنية وحدها كافية.', { cost: 2, pressure: -1, quality: 3 });
        } else if (choice === 'keep') {
          addDecision(`data-keep-${item.type}`, 'احتفظت بمادة رغم أن حالة استخدامها لم تُحسم', 'مررت المادة إلى الدفعة قبل اكتمال التحقق من الحقوق أو الخصوصية.', { cost: -1, pressure: 1, quality: -3 });
        }
      }
  ...
  function dataCleanSummary() {
    addLedger(3, 'منتجو المحتوى + نور', 'إنتاج مواد أصلية ثم جمع وفرز وتنظيف ومراجعة الجودة والخصوصية وحقوق الاستخدام', 'بيانات مجهزة للتطوير', 'هذه ليست بالضرورة المجموعة النهائية للتدريب؛ قد تنتقل بعض المواد إلى تصنيف بشري أو ضبط أو تقييم قبل استخدامها.');
  ...
  function abstract4() {
    abstraction([['كتّاب ومستخدمون', 'إنتاج المحتوى', '✍'], ['نور', 'تجهيز البيانات', '◫']], 'بيانات مجهزة للتطوير', 'المحتوى وقرارات الجمع والتنظيف والمراجعة أصبحت في النظام مواد بيانات جاهزة للانتقال إلى تصنيف أو تدريب أو تقييم.', 'ch5Intro');
  }
