export function createTrainingRoutes(ctx) {
  ...
  function trainingSetup() {
    ...
    html(`<div><span class="eyebrow">مختبر تطوير نموذج افتراضي</span><h1 class="scene-title">أنت الآن ديفيد، مهندس تعلم آلي.</h1><div class="reality-note"><strong>ما الذي نحاكيه تحديدًا؟</strong> هذه ليست عملية تدريب نموذج من الصفر. السيناريو يمثل جولة تدريب إضافية/ضبط لاحق تبدأ من نقطة حفظ سابقة، لذلك يظهر checkpoint وتظهر أمثلة بشرية جاهزة.</div><div class="training-board"><div class="config-panel"><div class="form-row"><label>مواد التطوير</label><select disabled><option>الدفعة 18 + أمثلة بشرية — جاهزة</option></select></div><div class="form-row"><label for="computeSel">الخوادم المخصصة</label><select id="computeSel"><option value="12" ${state.flags.trainingCompute==='12'?'selected':''}>12 مجموعة — تكلفة أعلى وهامش أعطال أكبر</option><option value="8" ${state.flags.trainingCompute==='8'?'selected':''}>8 مجموعات — تكلفة أقل وهامش أعطال أضيق</option></select></div><div class="form-row"><label for="checkpointSel">نقطة الحفظ</label><select id="checkpointSel"><option value="validated" ${checkpoint==='validated'?'selected':''}>اختُبرت على نطاق أوسع — تحقق أقل مطلوبًا الآن</option><option value="recent" ${checkpoint==='recent'?'selected':''}>أحدث — تحتاج تحققًا إضافيًا من تغييراتها</option></select></div><button id="trainStart" class="primary-btn training-start">ابدأ الجولة</button></div><div class="chart-panel"><span class="kicker">ما الذي سيتغير فعلًا؟</span><div class="training-log training-config-summary">12 مجموعة: فقد وحدة يترك 11 مجموعة متاحة، أي هامش أعطال أكبر.<br>8 مجموعات: فقد وحدة يترك 7 مجموعات متاحة، أي هامش أضيق.<br>نقطة أكثر اختبارًا: تحقق إضافي أقل.<br>نقطة أحدث: تحقق إضافي أكبر قبل الإطلاق.</div></div></div></div>`);
    $('#trainStart').addEventListener('click',()=>{
      ...
      if(state.flags.trainingCompute==='12') addDecision('training-compute-12','خصصت 12 مجموعة خوادم لجولة التطوير','دفعت تكلفة حوسبة أعلى مقابل هامش أكبر إذا خرجت وحدة من الخدمة.');
      else addDecision('training-compute-8','خصصت 8 مجموعات خوادم لجولة التطوير','خفضت تكلفة الحوسبة، لكن فقد وحدة واحدة ترك هامشًا أضيق.');
      if(state.flags.trainingCheckpoint==='validated') addDecision('training-checkpoint-validated','بدأت من نقطة حفظ اختُبرت أكثر','احتاجت نقطة البداية إلى تحقق إضافي أقل في هذه الجولة، من دون افتراض أنها أفضل لغويًا.');
      else addDecision('training-checkpoint-recent','بدأت من نقطة حفظ أحدث','استخدمت تغييرات أحدث مع حاجة إلى اختبارات تحقق إضافية قبل الإطلاق.');
  ...
  function trainingRun() {
    ...
    $('#trainPause').addEventListener('click',()=>{
      ...
      addDecision('train-pause','أوقفت جولة التطوير لتشخيص العطل','تحملت تأخيرًا، عزلت الوحدة المعطلة، ثم استأنفت الجولة بعد التحقق.');
  ...
  function trainingEval() {
    addLedger(5,'ديفيد وفرق التطوير','إعداد وتشغيل ومراقبة جولة تدريب إضافي وحل أعطال الحوسبة','نسخة مطورة من النموذج','يمثل السيناريو جولة post-training مبسطة، لا تدريب نموذج كامل من الصفر.');
