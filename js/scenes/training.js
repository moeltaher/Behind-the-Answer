...
export function createTrainingRoutes(ctx) {
  ...
  function trainingEligibilityMarkup() {
    ...
    return `<section class="card"><span class="kicker">بوابة أهلية البيانات قبل المعالجة</span><h2>المرور في مرحلة البيانات لا يمنح إذنًا تلقائيًا لاستخدام المادة في التدريب الإضافي.</h2><p>اتخذ قرارًا مستقلًا لكل مادة غير محسومة. السماح هنا يعني اعتمادها كمدخل للجولة التالية، لكنه لا يحول المشكلة إلى «محسومة». وإذا عولجت المادة فعلًا فسيبقى ذلك في التاريخ حتى لو حُذفت من نسخة لاحقة.</p><div class="evidence-results">${unresolved.map(index=>{
  ...
  function trainingSetup() {
    ...
    html(`<div><span class="eyebrow">مختبر تطوير نموذج افتراضي</span><h1 class="scene-title">أنت الآن ديفيد، مهندس تعلم آلي.</h1><div class="reality-note"><strong>ما الذي نحاكيه تحديدًا؟</strong> هذه ليست عملية تدريب نموذج من الصفر. السيناريو يمثل جولة تدريب إضافي مبسطة تبدأ من نقطة حفظ سابقة. أهلية المادة للجولة قرار مستقل عن مرورها داخل مسار العمل.</div>${trainingEligibilityMarkup()}${eligibilityDone&&!hasInputs?'<div class="alert dangerish" role="status"><strong>لا يمكن بدء جولة بلا مدخلات</strong><span>لا توجد مادة بيانات مؤهلة ولا مثال تصنيف بشري مؤكد. غيّر قرار أهلية مادة، أو ارجع في رحلة جديدة إلى مرحلة البيانات/التصنيف.</span></div>':''}<div class="training-board"><div class="config-panel"><div class="form-row"><label for="trainingInputsSummary">مدخلات الجولة الحالية</label><select id="trainingInputsSummary" disabled><option>${trainingData.passed} مواد (${trainingData.clear} محسومة / ${trainingData.unresolved} مسموح بها رغم حاجب) + ${confirmedLabel}</option></select></div><div class="form-row"><label for="computeSel">مجموعات الحوسبة المخصصة</label><select id="computeSel"><option value="12" ${state.flags.trainingCompute==='12'?'selected':''}>12 مجموعة — هامش سعة أكبر</option><option value="8" ${state.flags.trainingCompute==='8'?'selected':''}>8 مجموعات — تخصيص أقل وهامش أضيق</option></select></div><div class="form-row"><label for="checkpointSel">نقطة الحفظ</label><select id="checkpointSel"><option value="validated" ${checkpoint==='validated'?'selected':''}>أكثر اختبارًا — تحقق إضافي أقل</option><option value="recent" ${checkpoint==='recent'?'selected':''}>أحدث — تغيير مستهدف للنبرة العربية</option></select></div><button id="trainStart" class="primary-btn training-start" ${canStart?'':'disabled'}>${canStart?'ابدأ الجولة':blocker}</button></div><div class="chart-panel"><span class="kicker">ما الذي ستلاحظه من الإعداد؟</span><div class="training-log training-config-summary" tabindex="0" role="region" aria-label="ملخص إعداد جولة التدريب"><strong>الحوسبة</strong><br>يفترض السيناريو أن الجولة تحتاج إلى ${MIN_COMPUTE_TO_CONTINUE} مجموعات متاحة على الأقل للاستمرار. التخصيص الأكبر يترك هامشًا أكبر عند خروج مجموعة؛ اللعبة تسجل حجم التخصيص لكنها لا تزعم وجود ميزانية لم تحاكها.<br><br><strong>البيانات والتصنيف</strong><br>${pendingLabel} و${annotation.rejected} مرفوضة خارج المدخل المؤكد. المادة الموقوفة لا تدخل النسخة المرشحة. المادة التي تسمح بها رغم حاجب تظل المشكلة عليها مسجلة.<br><br><strong>نقطة الحفظ</strong><br>الأحدث تحمل تغييرًا مقصودًا، لكن فائدته لن تُفترض مسبقًا؛ ستقارن أمثلة فعلية لاحقًا.</div></div></div></div>`);
    ...
    document.querySelectorAll('[data-training-use]').forEach(button=>button.addEventListener('click',()=>{
      ...
      addDecision(`data-training-override-${index}`,`سمحت للمادة ${index+1} بدخول التدريب الإضافي رغم بقاء حاجب حوكمة`,`هذا قرار أهلية مستقل؛ لا يحوّل الحقوق أو الخصوصية أو الملاءمة إلى محسومة.`);
    ...
    document.querySelectorAll('[data-training-hold]').forEach(button=>button.addEventListener('click',()=>{
      ...
      addDecision(`data-training-hold-${index}`,`أوقفت المادة ${index+1} قبل التدريب الإضافي`,`فصلت بين مرور المادة داخل مسار العمل وبين أهلية معالجتها في جولة التطوير.`);
      saveState(); trainingSetup();
    }));
    ...
    $('#trainStart')?.addEventListener('click',()=>{
      ...
      addDecision(`training-compute-${total}-r${revision}`,`خصصت ${total} مجموعة حوسبة للنسخة المرشحة ${revision}`,`سجلت الجولة تخصيص ${total} مجموعة مع هامش سعة أولي ${margin} فوق الحد الأدنى المفترض (${MIN_COMPUTE_TO_CONTINUE}). لا تستخدم اللعبة رقم التخصيص كتكلفة أو ميزانية غير ممثلة.`);
  ...
  function trainingRecovery() {
    const total=Number(state.flags.trainingCompute);
    const revision=state.flags.candidateRevision;
    html(`<div><span class="eyebrow">تشخيص عطل الحوسبة — النسخة المرشحة ${revision}</span><h1 class="scene-title">حددت فرق البنية مجموعة خرجت من الخدمة وتحتاج إلى استعادة قبل الاستئناف.</h1>${supportingRoleStrip(['infraTeam'],'من ينفذ الاستعادة؟')}<div class="dual-view"><div class="view-panel"><h3>قبل الإصلاح</h3><div class="view-list"><span>الجولة متوقفة عند 35%</span><span>مجموعة واحدة معزولة</span><span>${total-1}/${total} مجموعات صالحة</span></div></div><div class="view-panel"><h3>المطلوب قبل الاستئناف</h3><div class="view-list"><span>إعادة تشغيل المجموعة أو استبدالها</span><span>اختبار اتصالها بالمجموعة</span><span>التحقق من عودة ${total}/${total}</span></div></div></div><div class="action-row"><button id="repairTrainingCompute" class="primary-btn">استعد المجموعة وتحقق من عودتها</button></div></div>`);
    $('#repairTrainingCompute').addEventListener('click',()=>{
      state.flags.trainingIncidentChoice='pause';
      addDecision(`train-recovery-r${revision}`,`استعدت مجموعة الحوسبة قبل استئناف النسخة ${revision}`,`أعيدت المجموعة المعطلة أو استبدلت، وتحقق الفريق من عودة ${total}/${total} مجموعات إلى الحالة الصالحة قبل استئناف الجولة.`);
      saveState(); go('trainingEval');
    });
  }
  ...
  function trainingEval() {
    ...
    const revision=state.flags.candidateRevision;
    addLedger(5,'ديفيد وفرق التطوير','إعداد وتشغيل ومراقبة جولة تدريب إضافي وحل أعطال الحوسبة',`نسخة مرشحة ${revision}`,exposed?`تحتوي النسخة الحالية على ${exposed} مواد ذات مسائل غير محسومة؛ تاريخ الاستخدام محفوظ وأي إعادة تدريب ستنتج revision جديدة وتبطل الأدلة السابقة.`:`دخلت النسخة الحالية ${current.passed} مواد بيانات، إضافة إلى الأمثلة البشرية المؤكدة.`);
    const paused=state.flags.trainingIncidentChoice==='pause';
    const total=Number(state.flags.trainingCompute);
    const compute=computeDescription(total);
    html(`<div><span class="eyebrow">نتيجة جولة التطوير — النسخة المرشحة ${revision}</span><h1 class="scene-title">انتهت الجولة، لكن النسخة ما زالت تحتاج إلى تقييم وتحقق.</h1><div class="stage-output"><strong>أثر قرار السعة والعطل</strong>${paused?`توقفت الجولة، استعيدت المجموعة المعطلة بعد تشخيصها والتحقق منها، ثم استؤنفت مع ${total}/${total} مجموعة صالحة.`:`اكتملت الجولة بعد خروج مجموعة مع ${compute.afterFailure} متاحة و${compute.remainingMargin} فوق الحد الأدنى.`}</div><div class="stage-output"><strong>سلسلة البيانات</strong>النسخة الحالية تستخدم ${state.flags.dataCurrentTrainingUsed.length} مواد بيانات. السجل التاريخي يحتفظ بـ${state.flags.dataTrainingUsed.length} مواد سبق أن دخلت معالجة في أي نسخة مرشحة.</div>${exposed?`<div class="alert dangerish"><strong>حاجب قد يفرض نسخة مرشحة جديدة</strong><span>${exposed} مواد غير محسومة موجودة في النسخة الحالية. إذا لم تُحسم بدليل، يجب استبعادها ثم إعادة الجولة؛ وعندها تُبطل نتائج التقييم والسلامة والإصدار المرتبطة بهذه النسخة.</span></div>`:''}<div class="action-row"><button id="sendHuman" class="primary-btn">انتقل إلى التقييم</button></div></div>`);
    $('#sendHuman').addEventListener('click',()=>go('abstract6'));
  }

  function abstract6() {
    abstraction([['ديفيد','مهندس تعلم آلي','⌁'],['فرق البنية والتشغيل','','▥']],'نسخة مرشحة تحتاج تقييمًا','إعدادات الجولة والبيانات والحوسبة أصبحت نسخة مرشحة محددة؛ الأدلة اللاحقة تخص هذه النسخة ولا تنتقل تلقائيًا إلى نسخة يعاد تدريبها.','ch7Intro');
  }
