export function createTrainingRoutes(ctx) {
  ...
  function trainingSetup() {
    ...
    $('#trainStart').addEventListener('click', () => {
      ...
      if (state.flags.trainingCompute === '12') {
        ...
        addDecision(
          'training-compute-8',
          'خصصت 8 مجموعات خوادم للتدريب',
          'خفضت تكلفة الحوسبة، لكن الجولة أصبحت أبطأ وأكثر حساسية للأعطال.',
          { cost: -4, pressure: 4, burden: 2 }
        );
      }

      if (state.flags.trainingCheckpoint === 'validated') {
        addDecision(
  ...
  function trainingEval() {
    addLedger(
      5,
      'ديفيد وفرق التدريب',
      'إعداد وتشغيل ومراقبة جولات التدريب وحل أعطال الخوادم',
      'نسخة مدرَّبة من النموذج',
      'هذا السيناريو يمثل جولة تطوير مبسطة؛ مسارات التدريب الفعلية تختلف بين النماذج.'
    );
