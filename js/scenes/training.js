...
export function createTrainingRoutes(ctx) {
  ...
  function dataInputs(indices=null) {
    ...
    state.flags.dataStatuses.forEach((status,index)=>{
      ...
      if(hasUnresolved(state.flags.dataChecks[index])) unresolved+=1; else clear+=1;
    });
