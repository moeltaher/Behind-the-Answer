...
export function createDatacenterRoutes(ctx) {
  ...
  function dcCooling() {
    html(`<div><span class="eyebrow">إنذار التبريد أثناء الاختبار</span><h1 class="scene-title">وحدة التبريد رقم 3 لا تستجيب.</h1><p class="scene-subtitle">أوقف تشغيل المجموعة المتأثرة أولًا، ثم اختر هل تنقل مهام الاختبار إلى سعة سليمة في قاعة أخرى أم تنتظر إصلاح التبريد.</p><div class="reality-note"><strong>أرقام حمل السيناريو</strong> القاعة البديلة تعمل الآن عند حمل 68%، أي أن لديها هامشًا متاحًا قبل الوصول إلى الحد التشغيلي المفترض في اللعبة. إذا نقلت إليها مهام الاختبار سيرتفع الحمل إلى 82% ويبقى هامش 18%.</div><div class="monitor"><div class="monitor-tile"><span>حرارة المنطقة المتأثرة</span><strong class="numeric-value" dir="auto">33° م ↑</strong><div class="bar"><i class="meter-fill meter-fill--danger" style="width:88%"></i></div></div>${monitorTile('الحمل الحالي في القاعة البديلة','68%',68)}${monitorTile('هامش القاعة البديلة الآن','32%',32)}${monitorTile('الشبكة','مستقرة',70)}</div><div class="choice-grid"><button id="dcMove" class="choice-btn"><strong>اعزل المجموعة وشغّل الاختبار على سعة بديلة</strong><small>تتوقف الأجهزة المتأثرة، وتنتقل مهام الاختبار إلى القاعة السليمة فيرتفع حملها من 68% إلى 82%.</small></button><button id="dcStop" class="choice-btn"><strong>أوقف الاختبار حتى إصلاح التبريد</strong><small>يؤخر التجهيز، ويمنح فريق التبريد وقتًا لإصلاح السبب قبل استئناف المجموعة نفسها.</small></button></div></div>`);
    $('#dcMove').addEventListener('click',()=>{ state.flags.dcCoolingChoice='move'; addDecision('dc-move','عزلت المجموعة ونقلت مهام الاختبار','توقفت الأجهزة المتأثرة، وارتفع حمل القاعة البديلة من 68% إلى 82% مع بقاء هامش 18% في أرقام السيناريو.'); saveState(); go('dcCoolingOutcome'); });
  ...
  function dcWorkers() {
    ...
    $('#dcReady').addEventListener('click',()=>{ addLedger(2,'كارلوس وفرق المرافق','تركيب وطاقة وتبريد وشبكات وصيانة واختبار','خوادم متاحة للتشغيل','عرضت المرحلة الأدوار الداعمة كأشخاص مميزين وظيفيًا داخل شروط تشغيل الخادم.'); go('abstract3'); });
  }
