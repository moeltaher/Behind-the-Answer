...
export function createDatacenterRoutes(ctx) {
  ...
  function dcWorkers() {
    ...
    const cards=DATACENTER_WORKERS.map(({id,actorId,icon})=>{ const actor=supportingActor(actorId); const on=state.flags.revealedWorkers.includes(id); return `<button class="worker-person ${on?'revealed':''}" data-worker="${id}"><span class="person-icon">${icon}</span><strong>${ctx.h(actor.name)}</strong><small>${on?ctx.h(actor.role):'اكشف كيف يدعم هذا الدور تشغيل المركز'}</small></button>`; }).join('');
    html(`<div><span class="eyebrow">العمل المحيط بالخادم</span><h1 class="scene-title">من تحتاجه البنية حتى تعمل؟</h1><p class="scene-subtitle">اكشف ثلاثة أدوار مختلفة. الهدف ليس جمع بطاقات؛ لاحظ أن «الخادم الجاهز» يعتمد على مرافق وصيانة وأمن وتشغيل لا تظهر في لوحة الخدمة.</p><div class="worker-map">${cards}</div><div class="stage-output"><strong>ناتج المرحلة</strong>خوادم اجتازت التركيب والاختبار ويمكن تخصيص قدرتها للتدريب أو تشغيل الخدمة.</div><div class="action-row"><button id="dcReady" class="primary-btn" ${revealed<3?'disabled':''}>${revealed<3?`اكشف ${3-revealed} أدوار أخرى`:'أكملت المطلوب — تابع'}</button></div></div>`);
