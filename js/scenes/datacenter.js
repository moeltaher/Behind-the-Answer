...
export function createDatacenterRoutes(ctx) {
  ...
  function dcCooling() {
    html(`<div><span class="eyebrow">إنذار التبريد أثناء الاختبار</span><h1 class="scene-title">وحدة التبريد رقم 3 لا تستجيب.</h1>${supportingRoleStrip(['coolingTech'],'من يدخل مع كارلوس الآن؟')}<p class="scene-subtitle">أوقف تشغيل المجموعة المتأثرة أولًا، ثم اختر هل تنقل مهام الاختبار إلى سعة سليمة أم توقف الاختبار حتى يعمل فريق التبريد. في الحالتين لن تعتبر المجموعة الأصلية جاهزة قبل الإصلاح والقياس وإعادة الاختبار.</p><div class="reality-note"><strong>أرقام حمل السيناريو</strong> القاعة البديلة تعمل عند 68%. إذا نقلت إليها الاختبار سيرتفع الحمل إلى 82% ويبقى هامش 18%.</div><div class="monitor"><div class="monitor-tile"><span>حرارة المنطقة المتأثرة</span><strong class="numeric-value" dir="auto">33° م ↑</strong><div class="bar"><i class="meter-fill meter-fill--danger" style="width:88%"></i></div></div>${monitorTile('الحمل الحالي في القاعة البديلة','68%',68)}${monitorTile('هامش القاعة البديلة الآن','32%',32)}${monitorTile('الشبكة','مستقرة',70)}</div><div class="choice-grid"><button id="dcMove" class="choice-btn"><strong>اعزل المجموعة وانقل مهام الاختبار</strong><small>يحافظ على سير الاختبار على سعة سليمة، لكن إصلاح الأصل يبقى مطلوبًا.</small></button><button id="dcStop" class="choice-btn"><strong>أوقف الاختبار وانتظر فريق التبريد</strong><small>يؤخر التجهيز؛ بعد القرار ستنفذ الإصلاح ثم القياس وإعادة الاختبار صراحة.</small></button></div></div>`);
    $('#dcMove').addEventListener('click',()=>{state.flags.dcCoolingChoice='move';state.flags.dcCoolingStage='open';addDecision('dc-move','عزلت المجموعة ونقلت مهام الاختبار','توقفت الأجهزة المتأثرة واستمرت مهام الاختبار في القاعة البديلة؛ بقي إصلاح المجموعة الأصلية مطلوبًا.');saveState();go('dcCoolingOutcome');});
    $('#dcStop').addEventListener('click',()=>{state.flags.dcCoolingChoice='stop';state.flags.dcCoolingStage='open';addDecision('dc-stop','أوقفت الاختبار حتى إصلاح التبريد','توقف الاختبار؛ القرار لم يصلح العطل، وبقي تنفيذ الإصلاح والقياس وإعادة الاختبار مطلوبًا.');saveState();go('dcCoolingOutcome');});
  }

  function dcCoolingOutcome() {
    const f=state.flags,moved=f.dcCoolingChoice==='move';
    if(!f.dcCoolingChoice){go('dcCooling');return;}
    if(f.dcCoolingStage==='open'){
      html(`<div><span class="eyebrow">القرار لا يساوي الإصلاح</span><h1 class="scene-title">${moved?'انتقلت مهام الاختبار، لكن المجموعة المتأثرة ما زالت معزولة.':'توقف الاختبار، لكن وحدة التبريد ما زالت تحتاج إلى إصلاح.'}</h1>${supportingRoleStrip(['coolingTech'],'من ينفذ الإصلاح؟')}<div class="dual-view"><div class="view-panel"><h2>المجموعة المتأثرة</h2><div class="view-list"><span>متوقفة ومعزولة</span><span>وحدة التبريد لم تُصلح بعد</span><span>لا تُحسب قدرة جاهزة</span></div></div><div class="view-panel"><h2>${moved?'القاعة البديلة':'الاختبار'}</h2><div class="view-list"><span>${moved?'الحمل بعد النقل: 82%':'متوقف أثناء الإصلاح'}</span><span>${moved?'الهامش المتبقي: 18%':'سيستأنف بعد التحقق'}</span></div></div></div><div class="action-row"><button id="repairCooling" class="primary-btn">أصلح وحدة التبريد</button></div></div>`);
      $('#repairCooling').addEventListener('click',()=>{f.dcCoolingStage='repaired';addDecision('dc-cooling-repaired','أصلحت وحدة التبريد','نُفذ الإصلاح، لكن المجموعة لم تعد جاهزة بعد؛ بقي قياس الحرارة وإعادة الاختبار.');saveState();dcCoolingOutcome();});return;
    }
    if(f.dcCoolingStage==='repaired'){
      html(`<div><span class="eyebrow">التحقق بعد الإصلاح</span><h1 class="scene-title">الإصلاح نُفذ؛ أثبت الآن أن المجموعة عادت إلى النطاق.</h1>${supportingRoleStrip(['coolingTech'],'من ينفذ القياس وإعادة الاختبار؟')}<div class="hud-grid"><div class="hud-item"><span>الحرارة قبل الإصلاح</span><strong>33° م</strong></div><div class="hud-item"><span>الحالة الحالية</span><strong>تنتظر القياس</strong></div></div><div class="action-row"><button id="verifyCooling" class="primary-btn">قِس الحرارة وأعد اختبار المجموعة</button></div></div>`);
      $('#verifyCooling').addEventListener('click',()=>{f.dcCoolingStage='verified';addDecision('dc-cooling-close','أغلقت عطل التبريد بعد قياس وإعادة اختبار صريحين','عادت الحرارة إلى 24° م ثم اجتازت المجموعة إعادة الاختبار قبل إعادتها للخدمة.');saveState();dcCoolingOutcome();});return;
    }
    html(`<div><span class="eyebrow">إغلاق عطل التبريد</span><h1 class="scene-title">أُصلحت وحدة التبريد ثم اجتازت المجموعة إعادة الاختبار.</h1>${supportingRoleStrip(['coolingTech'],'من أغلق العطل؟')}<div class="dual-view"><div class="view-panel"><h2>المجموعة المتأثرة</h2><div class="view-list"><span>عادت الحرارة إلى 24° م</span><span>اجتازت إعادة الاختبار</span><span>يمكن إعادتها إلى الخدمة</span></div></div><div class="view-panel"><h2>${moved?'مسار الاختبار البديل':'مهام الاختبار'}</h2><div class="view-list"><span>${moved?'حافظ على استمرار العمل أثناء الإصلاح':'توقفت أثناء الإصلاح ثم استؤنفت'}</span><span>لم يُستخدم بديلًا عن إصلاح الأصل</span></div></div></div><div class="action-row"><button id="dcAfterCooling" class="primary-btn">تابع إلى فرق التشغيل</button></div></div>`);
    $('#dcAfterCooling').addEventListener('click',()=>go('dcWorkers'));
  }

  function dcWorkers() {
    if(state.flags.dcCoolingStage!=='verified'){go('dcCoolingOutcome');return;}
    ...
    $('#dcReady').addEventListener('click',()=>{addLedger(2,'كارلوس وفرق المرافق','تركيب وطاقة وتبريد وشبكات وصيانة واختبار','خوادم متاحة للتشغيل','لم تُحسب المجموعة المتأثرة جاهزة حتى أُصلح التبريد وقيس أثر الإصلاح واجتازت إعادة الاختبار.');go('abstract3');});
  }
