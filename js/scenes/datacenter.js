...
export function createDatacenterRoutes(ctx) {
  ...
  function dcCooling() {
    ...
    $('#dcStop').addEventListener('click',()=>{state.flags.dcCoolingChoice=null;state.flags.dcCoolingRestored=false;addDecision('dc-stop','أوقفت الاختبار حتى إصلاح التبريد','توقف الاختبار؛ القرار لم يصلح العطل، وبقي تنفيذ الإصلاح والتحقق وإعادة الاختبار مطلوبًا.');saveState();go('dcCoolingOutcome');});
  }

  function dcCoolingOutcome() {
    const moved=state.flags.dcCoolingChoice==='move';
    const stoppedPending=state.flags.dcCoolingChoice===null&&state.decisions.some(decision=>decision.id==='dc-stop');
    if(!state.flags.dcCoolingRestored){
      ...
      $('#repairCooling').addEventListener('click',()=>{if(stoppedPending)state.flags.dcCoolingChoice='stop';state.flags.dcCoolingRestored=true;addDecision('dc-cooling-close','أغلقت عطل التبريد بعد إصلاح واختبار صريح','أصلح فريق التبريد السبب، تحقق من الحرارة، ثم أعاد اختبار المجموعة قبل إعادتها للخدمة.');saveState();dcCoolingOutcome();});
      return;
    }
