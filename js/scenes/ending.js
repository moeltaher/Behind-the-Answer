...
const BASE_SUPPORTING_IDS = ['transportTeam','maintenance','qualityInspector','cleaner','securityWorker','coolingTech','electricalEngineer','cableTech','networkOperator','contentCreators','dataReviewer','infraTeam','safetyTester','releaseManager','operationsTeam','affectedUser'];
...
const TRANSFER_TASKS = [
  ...
  ['retrieval','في خدمة تستخدم الاسترجاع المعزز بالتوليد (RAG): استرجاع مستندات مرتبطة بسؤال المستخدم الحالي','request'],
...
function dataLineageSummary(state) {
  ...
  const rows=state.flags.dataStatuses.map((status,index)=>{
    ...
    if(current.has(index)) lineage=`ضمن النسخة المرشحة ${state.flags.candidateRevision} الحالية`;
    ...
    else if(status==='pending') lineage='معلقة في مسار العمل ولم تدخل التدريب';
...
function encounteredSupportingActors(state) {
  const ids=[...BASE_SUPPORTING_IDS];
  if(state.decisions.some(decision=>decision.id==='data-pii-redact-after-review'||decision.id==='data-pii-keep-after-review')) ids.push('rightsReviewer');
...
export function createEndingRoutes(ctx) {
  ...
  function results() {
    ...
    const supportingPeople=encounteredSupportingActors(state);
    ...
    const materialHighlight=priorityDecision(materialAndData,['data-retrain-plan-*','data-license-evidence-*','data-training-override-*','data-pii-keep-after-review','data-pii-redact-after-review','dc-cooling-close','dc-move','dc-stop','factory-debt-closed','factory-debt-carried','factory-maintenance-closed','factory-stop','factory-continue','data-*']);
    const trainingHighlight=priorityDecision(trainingAndLaunch,['launch-delay*','launch-fast*','launch-ready*','release-gate-*','checkpoint-evidence-reviewed*','safety-second-review*','safety-caught*','train-recovery*','train-continue*','train-pause*']);
    const operationsHighlight=priorityDecision(operations,['deploy-resilience-risk-*','monitoring-check-*','deploy-rollback','deploy-restart','deploy-failover-review-*','deploy-capacity-load-*','support-*']);
