...
const PIPELINE_STEPS = [
  'استخراج المواد', 'صناعة الأجهزة', 'مراكز البيانات', 'تجهيز البيانات',
  'تصنيف البيانات', 'تدريب النموذج', 'المراجعة البشرية', 'تشغيل الخدمة'
];
...
const ENDING_COPY = {
  ...
  invisible: ['سلسلة قليلة الظهور', 'ظلت أعمال بشرية مهمة غير ظاهرة داخل القرارات والمؤشرات التي صنعتها السلسلة.'],
...
function renderPipeline() {
  return PIPELINE_STEPS.map((step, index) => `<div class="pipeline-step">${step}</div>${index < PIPELINE_STEPS.length - 1 ? '<div class="pipeline-arrow">↓</div>' : ''}`).join('');
}
...
export function createEndingRoutes(ctx) {
  ...
  const { setChapter, chapterIntro, html, go, saveState, addLedger, renderLedger, resetGame } = ctx;
  ...
  function results() {
    ...
    html(`<div><span class="eyebrow">ماذا حدث في سلسلتك؟</span><h1 class="display-title">${h(title)}</h1><p class="scene-subtitle">${h(copy)}</p><div class="results-grid">${metric('ضغط الإنتاج', metrics.pressure, metricLabel(metrics.pressure))}${metric('تكلفة الشركة', metrics.cost, metricLabel(metrics.cost))}${metric('عبء العامل', metrics.burden, metricLabel(metrics.burden))}${metric('جودة الناتج', metrics.quality, metricLabel(metrics.quality))}${metric('ظهور العمل البشري داخل السلسلة', metrics.visibility, metricLabel(metrics.visibility))}</div><div class="card flat discovery-summary"><h2>استكشافك للعبة</h2><p>هذا مؤشر منفصل عن نتيجة السلسلة: ${metricLabel(metrics.discovery || 0)}. يرتفع عندما تفتح أدوارًا أو أقسامًا اختيارية، لكنه لا يقرر ما إذا كانت السلسلة نفسها تُظهر العمال أم تخفيهم.</p></div><div class="card flat results-decisions"><h2>قراراتك</h2><div class="decision-list">${decisions}</div></div><div class="alert"><strong>كثير من القرارات لم يكن له حل بلا تكلفة.</strong><span>الحصة والعقد والموعد وأنظمة التقييم تحدد مساحة الاختيار قبل أن يتخذ العامل أو الفريق قراره.</span></div><div class="action-row"><button id="resultsLedger" class="secondary-btn">عرض دفتر السلسلة</button><button id="toFinalMessage" class="primary-btn">إلى الخاتمة</button></div></div>`);
