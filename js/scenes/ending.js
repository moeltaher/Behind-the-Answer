...
const SECONDARY_LABOR = [
  'عمال النقل والمعالجة', 'فرق الصيانة والفحص', 'عمال النظافة والأمن',
  'فنيو الكهرباء والتبريد والكابلات', 'مشغلو الشبكات', 'مختبرو السلامة', 'مراجعو اللغة'
];

const RECOGNITION_POSITIVE = new Set([
  'mine-stop', 'factory-stop', 'dc-stop', 'annotation-break', 'annotation-appeal',
  'train-pause', 'launch-delay', 'deploy-rollback',
  'data-review-code', 'data-review-ambiguous', 'data-review-foreign', 'data-review-forum'
]);

const RECOGNITION_NEGATIVE = new Set([
  'mine-continue', 'factory-continue', 'dc-move', 'annotation-noappeal',
  'train-continue', 'launch-fast', 'deploy-restart',
  'data-keep-code', 'data-keep-ambiguous', 'data-keep-foreign', 'data-keep-forum'
]);
...
function discoveryPercent(state) {
  const found = Math.min(6, state.flags.revealedWorkers.length)
    + Math.min(12, state.flags.dataOrigins.length)
    + Math.min(3, state.flags.deployTabs.length);
  return Math.round((found / 21) * 100);
}
...
function resultInterpretation(name, value) {
  ...
  const copies = {
    pressure: label === 'مرتفع' ? 'ضغط أكبر للحفاظ على السرعة والموعد.' : label === 'منخفض' ? 'مساحة أكبر للتوقف والفحص.' : 'توازن نسبي بين السرعة والتوقف.',
    cost: label === 'مرتفع' ? 'الشركة تحملت جزءًا أكبر من تكلفة الوقت والحوسبة والمراجعة.' : label === 'منخفض' ? 'جرى تقليل تكلفة الشركة في عدد أكبر من القرارات.' : 'التكلفة موزعة بين أكثر من طرف.',
    burden: label === 'مرتفع' ? 'جزء أكبر من الضغط والمخاطر انتقل إلى العمال.' : label === 'منخفض' ? 'انخفض العبء النسبي على العمال في قرارات أكثر.' : 'عبء العمل بقي متوسطًا عبر السلسلة.',
    quality: label === 'مرتفع' ? 'القرارات دعمت جودة واختبارًا أوسع.' : label === 'منخفض' ? 'تراكمت مخاطر أكبر على جودة الناتج.' : 'الجودة مقبولة لكن بقيت تنازلات واضحة.',
    recognition: label === 'مرتفع' ? 'القرارات اعترفت أكثر بتكلفة العمل والوقت والمخاطر البشرية.' : label === 'منخفض' ? 'اختُصر العمل البشري أكثر داخل أهداف السرعة والكلفة.' : 'ظهر العمل في بعض القرارات واختفى في أخرى.'
  };
  return copies[name];
}
...
export function createEndingRoutes(ctx) {
  ...
  function finalAnswer() {
    ...
    const qualityNote = state.metrics.quality >= 70 ? 'الجودة المتراكمة في مسارك مرتفعة، لذلك تظهر نسخة أكثر ملاءمة وطبيعية.' : state.metrics.quality >= 45 ? 'بعض تنازلات الجودة ظهرت في صياغة أقل ملاءمة.' : 'تراكمت تنازلات كبيرة في الجودة، لذلك تظهر إجابة أضعف وأكثر عمومية.';
