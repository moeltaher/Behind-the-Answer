...
const STATE_NULLABLE_VALIDATORS = {
  ...
  dataFollowup: value => (
    ...
    Number.isInteger(value.index) && typeof value.reason === 'string'
  ),
...
function hasExactShape(template, value, key = '') {
  ...
  if (typeof template === 'object') {
    ...
    return templateKeys.every(childKey => Object.hasOwn(value, childKey) && hasExactShape(template[childKey], value[childKey], childKey));
  }
...
export function loadSettings() {
  try {
    ...
    const saved = JSON.parse(raw);
    return hasExactShape(DEFAULT_SETTINGS, saved) ? saved : { ...DEFAULT_SETTINGS };
