...
const FACTORY_REMEDIATION_STAGES=new Set(['none','diagnosed','verified']);
...
function validAnnotationResult(v){return exactKeys(v,['index','choice','acceptedAsReasonable','pending','reviewRejected','disputed'])&&Number.isInteger(v.index)&&v.index>=0&&v.index<6&&ANNOTATION_CHOICES.has(v.choice)&&['acceptedAsReasonable','pending','reviewRejected','disputed'].every(k=>typeof v[k]==='boolean');}
...
function hasCurrentTrainingConfig(state){const f=state.flags,r=f.candidateRevision;if(r<1)return false;return state.decisions.some(d=>d.id===`training-compute-${TRAINING_COMPUTE}-r${r}`)&&state.decisions.some(d=>d.id===`training-checkpoint-${f.trainingCheckpoint}-r${r}`);}
...
function sceneConsistent(state){
  ...
  if(TRAINING_OR_LATER.has(s)&&!hasCurrentTrainingConfig(state))return false;
...
function validStateInvariants(state){
  ...
  const resolvedFollowup=state.decisions.some(d=>d.id==='data-pii-redact-after-review'||d.id==='data-pii-keep-after-review')?1:0;
  const pendingFollowup=f.dataFollowup?1:0;
  if(sortActions!==f.dataIndex+resolvedFollowup+pendingFollowup)return false;
  ...
  if(f.dcCoolingRestored&&f.dcCoolingChoice===null)return false;
  if(f.miningForcedInspection&&!f.miningWarning)return false;
  if(f.factoryChoice===null&&(f.factoryMaintenanceDebt||f.factoryRemediationStage!=='none'))return false;
  if(f.factoryRemediationStage==='verified'&&f.factoryMaintenanceDebt)return false;
  if(f.factoryRemediationStage==='diagnosed'&&!f.factoryMaintenanceDebt)return false;
  if(f.factoryChoice==='stop'&&!f.factoryMaintenanceDebt&&f.factoryRemediationStage!=='verified')return false;
  ...
  if(f.deployRecovery!==null&&(!f.deployLoad||f.deployTabs.length!==3||f.deployFailoverChecks.length!==3||f.launchChoice===null||!hasResilienceResolution(state)))return false;
  ...
  if(f.deployRecovery===null&&f.deployRecoveryDisposition!==null)return false;
  ...
  const allDataRefs=[...f.dataTrainingUsed,...f.dataTrainingApproved,...f.dataCurrentTrainingUsed,...f.dataTrainingHeld];
  ...
  if(f.launchChoice!==null){if(!f.safetyRetested||f.releaseGates.length!==RELEASE_GATES.size)return false;if(exposedCurrentUnresolvedIndices(f).length)return false;}
...
function validState(state){
  ...
  const f=state.flags;
  const keys=['miningCount','miningMinutes','miningBUses','miningWarning','miningIncidentChoice','miningRiskLevel','miningForcedInspection','miningInspectionCount','factoryChoice','factoryMaintenanceDebt','factoryRemediationStage','serverSteps','dcCoolingChoice','dcCoolingRestored','dataOrigins','dataIndex','dataReviewMinutes','dataFollowup','dataStatuses','dataChecks','dataFeedbackLabel','dataFeedbackDetail','dataSort','dataTrainingApproved','dataTrainingUsed','dataCurrentTrainingUsed','dataTrainingHeld','annotationResults','annotationUnpaidMinutes','tookBreak','breakDecisionMade','candidateRevision','trainingCheckpoint','trainingIncidentChoice','evaluatorCalibrationComplete','checkpointEvalComplete','evalIndex','evalCorrectCount','evalFeedback','safetyChoice','safetyRemediated','safetyRetested','releaseGates','extraChecks','deferredExtraChecks','monitoringChecksCompleted','launchChoice','deployLoad','deployFailoverChecks','deployTabs','deployRecovery','deployRecoveryDisposition','supportIndex','supportFeedbackLabel','supportFeedbackDetail','transferChoice'];
  ...
  if(![f.miningWarning,f.miningForcedInspection,f.factoryMaintenanceDebt,f.dcCoolingRestored,f.tookBreak,f.breakDecisionMade,f.evaluatorCalibrationComplete,f.checkpointEvalComplete,f.safetyRemediated,f.safetyRetested].every(v=>typeof v==='boolean'))return false;
  if(!validNullableChoice(f.miningIncidentChoice,['stop','continue'])||!validNullableChoice(f.factoryChoice,['stop','continue'])||!FACTORY_REMEDIATION_STAGES.has(f.factoryRemediationStage)||!validNullableChoice(f.dcCoolingChoice,['move','stop']))return false;
  ...
  if(!['validated','recent'].includes(f.trainingCheckpoint)||!validNullableChoice(f.trainingIncidentChoice,['pause','continue']))return false;
  ...
  if(f.deployLoad!==null&&!(Array.isArray(f.deployLoad)&&f.deployLoad.length===3&&f.deployLoad.every(v=>Number.isInteger(v)&&v>=0&&v<=100)&&f.deployLoad.reduce((s,v)=>s+v,0)===100))return false;
  if(!uniqueIndices(f.deployFailoverChecks,3)||!uniqueAllowedStrings(f.deployTabs,DEPLOY_TABS)||!validNullableChoice(f.deployRecovery,['restart','rollback'])||!validNullableChoice(f.deployRecoveryDisposition,['monitor','cleared']))return false;
...
function normalizePreTraining(target){
  ...
  f.dataTrainingApproved=[];f.dataTrainingUsed=[];f.dataCurrentTrainingUsed=[];f.dataTrainingHeld=[];f.candidateRevision=0;f.trainingCheckpoint='validated';f.trainingIncidentChoice=null;f.evaluatorCalibrationComplete=false;f.checkpointEvalComplete=false;f.evalIndex=0;f.evalCorrectCount=0;f.evalFeedback=null;f.safetyChoice=null;f.safetyRemediated=false;f.safetyRetested=false;f.releaseGates=[];f.extraChecks=[];f.deferredExtraChecks=[];f.monitoringChecksCompleted=[];f.launchChoice=null;f.deployLoad=null;f.deployFailoverChecks=[];f.deployTabs=[];f.deployRecovery=null;f.deployRecoveryDisposition=null;f.supportIndex=0;f.supportFeedbackLabel='';f.supportFeedbackDetail='';f.transferChoice=null;
...
function migrateFactoryState(migrated,saved){
  ...
  if(f.factoryChoice==='stop'){
    if(saved.scene==='factoryOutcome'){f.factoryMaintenanceDebt=true;f.factoryRemediationStage='none';}
    else{f.factoryMaintenanceDebt=false;f.factoryRemediationStage='verified';}
    return;
  }
  if(f.factoryMaintenanceDebt){f.factoryRemediationStage='none';return;}
  f.factoryRemediationStage='verified';
}
function migrateOldState(defaultState,saved){
  if(!isPlainObject(saved)||![5,4,3,2,undefined].includes(saved.schemaVersion)||!isPlainObject(saved.flags))return null;
  ...
  try{
    ...
    if(saved.schemaVersion===5&&migrated.scene==='dcCoolingOutcome'&&migrated.flags.dcCoolingChoice===null&&!migrated.flags.dcCoolingRestored&&migrated.decisions.some(d=>d.id==='dc-stop'))migrated.flags.dcCoolingChoice='stop';
    ...
    const source=saved.schemaVersion===5?'الإصدار 5':saved.schemaVersion===4?'الإصدار 4':saved.schemaVersion===3?'الإصدار 3':saved.schemaVersion===2?'الإصدار 2':'إصدار قديم';
    migrated.systemNotice=rewound?`تم تحديث الحفظ من ${source} إلى الإصدار 6. احتفظت اللعبة بالتقدم حتى ما قبل التدريب الإضافي، وأعادتك إلى إعداد الجولة لأن التصميم الحالي أزال خيارات تشغيل قديمة ويفرض أدلة صريحة لكل نسخة مرشحة.`:`تم تحديث الحفظ من ${source} إلى الإصدار 6 مع الاحتفاظ بالتقدم الحالي.`;
